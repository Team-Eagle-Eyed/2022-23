...
public class AutoBalance extends Command {
    ...
    public AutoBalance(Swerve subsystem) {
        ...
        pitchController.setTolerance(7);
        rollController = new PIDController(kP, kI, kD);
        rollController.setTolerance(7);
      }
    ...
    public void execute() {
        ...
        if(Math.abs(pitchTiltSpeed) > 0.2 || Math.abs(currentPitch) < 10) {
            pitchController.setP(0);
        } else {
            pitchController.setP(kP);
        }
        pitchOutput = MathUtil.clamp(pitchController.calculate(currentPitch), -maxOutput, maxOutput);
        ...
        if(Math.abs(rollTiltSpeed) > 0.2 || Math.abs(currentRoll) < 10) {
            rollController.setP(0);
        } else {
            rollController.setP(kP);
        }
        rollOutput = MathUtil.clamp(rollController.calculate(currentRoll), -maxOutput, maxOutput);
        ...
        if(passCount >= 50) {
            s_Swerve.drive(new Translation2d(0, 0), 0.15, true, true);
            this.cancel();
        }
    }
}
